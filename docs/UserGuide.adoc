...
image::AnalyticsScreenshot.png[width="600"]

===== Summary
The summary displays of occurrences of events such as the number of appointments as in the screenshot above. They can be
 raw totals or averages. This will be indicated by the title. The numbers are categorized according to the following
 time periods:
...
* The number of appointments scheduled in the current year.
****
Format: `apptstats`
