...
the `patientQueue` underneath. In the scenario where a patient has a preferred doctor, the patient will be inserted into the
`preferenceQueue`. +
...
Step 5: The user keeps adding patients until there are 5 patients in the queue. The `patientQueue` and `preferenceQueue` underneath look like this: +
...
* **Alternative 2:** Insert the patient into only one queue.
** Pro: Uses less memory as only one data structure is needed.
** Con: Time complexity is slightly worse than the current implementation.
...
** Con: Time complexity of looking for a particular patient is worse.
...
`ExportPatientData`. All exported filed are stored in the root folder of the program.
...
The login mechanism facilitated by using `UniqueDoctorList` or `UniqueReceptionistList` to retrieve a list of doctors or receptionist based on the user's role. +

These operations are exposed in the Model interface as Model#hasDoctor() and Model#getDoctor() for doctors
or Model#hasReceptionist() and Model#getReceptionist() for receptionists respectively.

Given below is an scenario to illustrate the login authentication at each step: +

. The user executes the `login` command by specifying the role, name and password. The `login` command determines the type of role entered by the user.

. If the role is doctor,

.. The `login` command call `Model#hasDoctor()` to ensure that this doctor exist inside ClinicIO.
.. Once ClinicIO check if user exists, the `login` command call `Model#getDoctor` to retrieve the doctor found in `UniqueDoctorList`.
.. The `login` command verifies a password using `HashUtil#verifyPassword()` with the given doctor's password and password entered by the user.
.. To indicate successful completion of verification, the `login` command raises a `SuccessfulLoginEvent`.

. If the role is receptionist,

.. the `login` command invokes `Model#hasReceptionist()` to ensure that this receptionist exists inside ClinicIO.
.. Once ClinicIO has checked that the user exists, the `login` command invokes `Model#getReceptionist()` to retrieve the receptionist found in `UniqueReceptionistList`.
.. The `login` command verifies a password using `HashUtil#verifyPassword()` with the given receptionist's password and password entered by the user.

. Once verification has been made successfully, the `login` command raise an `SuccessfulLoginEvent` to indicate that a successful attempt to login has been made.
