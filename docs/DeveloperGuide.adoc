...
wrapper class for the various statistics and employs the delegation pattern. Each of statistics classes depend on
`DateUtil` and `TimeUtil`. These classes contain utility methods for date and time information such as the number of
occurrences in a certain date or time period. The occurrences can be of events such as the number of appointments.

Additionally, each of the statistics classes depend on the following classes for storing data:
...
** used mostly within a `List`.


The following is the general sequence of processes for the different types of statistics:
