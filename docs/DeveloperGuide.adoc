...
image::AnalyticsClassDiagram.png[width="500"]
...
*** `dataGroups`: the groups of data points to be plotted. [red]*Must* be the same in number as `dataGroupsLabels`.
*** `dataGroupsLabels`: the label for each group of data. Used in the legend of the chart. [red]*Must* be the same in
number as
 `dataGroups`.
* `Tuple`:
...
** Con: This increases the computational workload which may provide little benefit if statistics are not viewed
...
** Cons: The burden is placed on the user having to decide which statistics they want to view. It is more difficult to
implement as a consistent user interface needs to be designed to accomodate all the possible combinations.
// end::analytics[]
...
image::ExportSwimlaneDiagram.png[width="500"]
