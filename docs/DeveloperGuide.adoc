...
=== Analytics feature
==== Overview
The analytics feature allows users to view data across the various functions of a clinic. These are viewed in isolation
with separate commands. However, they share a similar user interface composed of two distinct sections. They are the
*summary* and the *visualization* as shown in the screenshot below.

===== Summary
The summary displays of occurrences of events such as the number of appointments as in the screenshot above. They can be
 raw totals or averages. This will be indicated by the
title.
The numbers are categorized according to the following time periods:
